...
| Name | Source | Description |
|------|--------|-------------|
| **mattpocock-skills** | Store | Matt Pocock 出品的工程技能集合。包含审查确认（grill-me/grill-with-docs）、测试驱动开发（tdd）、诊断调试（diagnose）、架构改进（improve-codebase-architecture）、原型设计（prototype）等专业工程实践技能。 |
| **summarize** | Store | 快速摘要 CLI 工具。支持对网页 URL、PDF 文档、图片、音频文件、YouTube 链接等内容一键生成摘要。当用户需要快速获取网页/文档/视频的核心内容摘要时使用。 |

---

**总计**: 30 | **来源**: skills-manager central repository
